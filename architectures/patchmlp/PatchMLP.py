...
import numpy as np
from patchmlp.layers.Embed import Emb
...
class Model(nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_uniform_(self.projector.weight, gain=1e-9)  # weight scale (gain)
...
class Encoder(nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_uniform_(self.ff1[0].weight, gain=1e-9)  # weight scale (gain)
        nn.init.constant_(self.ff1[0].bias, 0)
        nn.init.xavier_uniform_(self.ff2[0].weight, gain=1e-9)  # weight scale (gain)
