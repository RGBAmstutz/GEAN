...
class H36MEval(data.Dataset):
    ...
    def _get_h36m_files(self):
        ...
        try:
            with open(os.path.join(self._h36m_anno_dir.replace('h36m', ''), "h36m_test.txt"), 'r') as file:
                seq_names += file.readlines()
        except Exception as e:
            print(f"Error reading h36m_test.txt: {e}")
            return
        ...
        for subject in seq_names:
            ...
            for act in self._actions:
                ...
                tmp_data_idx_3 = [act] * len(valid_frames0)
                ...
                tmp_data_idx_3 = [act] * len(valid_frames1)
                self.data_idx.extend(zip(tmp_data_idx_1, tmp_data_idx_2, tmp_data_idx_3))
                idx += 2
    ...
    def __getitem__(self, index):
        idx, start_frame, action = self.data_idx[index]
        frame_indexes = np.arange(start_frame,
                                  start_frame + self.h36m_motion_input_length + self.h36m_motion_target_length)
        ...
        return h36m_motion_input, h36m_motion_target, action
