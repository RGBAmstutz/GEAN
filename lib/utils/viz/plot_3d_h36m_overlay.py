...
def plot_3d_motion(gt, pred, out_name, title, figsize=(10, 10), fps=120, radius=4, alt_color=False):
    matplotlib.use('Agg')
    pred_data = pred.copy().reshape(len(pred), -1, 3)
    gt_data = gt.copy().reshape(len(gt), -1, 3)
    ...
    colors_pred = ['red', 'blue', 'black', 'red', 'blue',
              'darkblue', 'darkblue', 'darkblue', 'darkblue', 'darkblue',
              'darkred', 'darkred', 'darkred', 'darkred', 'darkred']

    colors_gt = ['grey', 'grey', 'grey', 'grey', 'grey',
    ...
    def update(index):
        ...
        # Plot the 3D skeleton (ground truth)
        for i, (chain, color) in enumerate(zip(h36m_kinetic_chain, colors_pred if not alt_color else colors_gt)):
            linewidth = 3.0 if i < 5 else 1.0
            ax.plot3D(gt_data[index, chain, 0], gt_data[index, chain, 1], gt_data[index, chain, 2], linewidth=linewidth, color=color)

        # Plot the 3D skeleton
        for i, (chain, color) in enumerate(zip(h36m_kinetic_chain, colors_pred if not alt_color else colors_alt)):
            linewidth = 4.0 if i < 5 else 2.0
            ax.plot3D(pred_data[index, chain, 0], pred_data[index, chain, 1], pred_data[index, chain, 2], linewidth=linewidth, color=color)
...
def draw_to_batch(gt, smpl_joints_batch, title_batch=None, outname=None, alt_color=False, figsize=(10,10)):
    batch_size = len(smpl_joints_batch)
    out = []
    for i in range(batch_size):
        out.append(plot_3d_motion(
            gt=gt[i],
            pred=smpl_joints_batch[i],
            out_name=None,
