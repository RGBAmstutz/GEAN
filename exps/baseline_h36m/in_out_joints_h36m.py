...
def main():
    ...
    model.load_state_dict(torch.load(args.model_pth, weights_only=True), strict=True)
    model.eval()
    model.cuda()
    ...
    dct_m, idct_m = get_dct_matrix(config.motion.h36m_input_length_dct)  # assuming a suitable input length is set in config
