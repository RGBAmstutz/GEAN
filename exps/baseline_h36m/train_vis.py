...
parser.add_argument('--dct', action='store_true', help='=enable dct')
...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    motion_pred = motion_pred.reshape(b, n, 28, 3).reshape(-1, 3)
    h36m_motion_target = h36m_motion_target.cuda().reshape(b, n, 28, 3).reshape(-1, 3)
    loss = torch.mean(torch.norm(motion_pred - h36m_motion_target, 2, 1))

    if config.use_relative_loss:
        motion_pred = motion_pred.reshape(b,n,28,3)
        dmotion_pred = gen_velocity(motion_pred)
        motion_gt = h36m_motion_target.reshape(b,n,28,3)
...
dataset = H36MDataset(config, 'train', config.data_aug, vis=True)
