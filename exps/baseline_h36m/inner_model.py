...
class MiniTrans(nn.Module):
    ...
    def reset_parameters(self):
        nn.init.xavier_uniform_(self.ff[0].weight, gain=1e-10)  # weight scale (gain)
        nn.init.constant_(self.ff[0].bias, 0)
        nn.init.xavier_uniform_(self.ff[2].weight, gain=1e-10)  # weight scale (gain)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        if self.temporal_fc_in:
            if config.fft:
                self.motion_fc_in = nn.Linear(self.config.motion.h36m_input_length_dct, self.config.motion.h36m_input_length_dct, dtype=cfloat)
            else:
                if config.conv:
                    self.conv = nn.Conv1d(self.config.motion.h36m_input_length_dct, self.config.motion.h36m_input_length_dct, kernel_size=self.config.kernel_size, padding=self.config.pad)
                if config.att_out:
            ...
            if config.fft:
                ...
                if config.conv:
                    self.conv = nn.Conv1d(self.config.motion.h36m_input_length, self.config.motion.h36m_input_length, kernel_size=self.config.kernel_size, padding=self.config.pad)
                if config.att_out:
        ...
        if self.temporal_fc_out:
            if config.ifft:
                self.motion_fc_out = nn.Linear(self.config.motion.h36m_input_length_dct, self.config.motion.h36m_input_length_dct, dtype=cfloat)
            else: 
