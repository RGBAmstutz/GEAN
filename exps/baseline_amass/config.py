...
add_path(osp.join(C.root_dir, 'kan'))
add_path(osp.join(C.root_dir, 'architectures'))
...
C.pad = 1
C.pool = 'None'
C.pool_in = int(C.motion.dim / C.kernel_size)
C.pool_in_dct = int(C.motion.amass_input_length_dct / C.kernel_size)
C.att_out = False
C.att_in = False
## Core of the model (apart from dct and other minor transformations)
dim_ = C.motion.dim
C.core = edict()
C.core.hidden_dim = dim_
C.core.seq_len = C.motion.amass_input_length_dct
C.core.num_layers = 48 # default 48
C.core.with_normalization = True
C.core.spatial_fc_only = False
C.core.norm_axis = 'spatial'
...
"""Train Config"""
C.batch_size = 256 # default 256
C.num_workers = 8 # default 8
